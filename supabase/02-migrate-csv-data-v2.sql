...
-- ============================================
-- Updated version: handles deadlines.csv without priority column
-- Automatically assigns priority for duplicate deadlines
...
-- Step 4: Fix schema to allow multiple deadlines
-- ============================================
...
-- Step 5: Migrate Deadlines with AUTO PRIORITY
-- ============================================
-- This uses ROW_NUMBER to automatically assign priority
-- for schools with multiple deadlines of the same type
...
select
  ...
  d.raw_value,
  row_number() over (
    partition by s.id, c.id, d.plan
    order by d.deadline_date nulls last
  ) as priority
...
-- Show schools with multiple deadlines per plan type
select
  s.school_name,
  sd.plan,
  count(*) as num_deadlines,
  array_agg(sd.deadline_date order by sd.priority) as dates
from public.school_deadlines sd
...
-- Step 6: Cleanup - Drop staging tables (optional)
-- ============================================
